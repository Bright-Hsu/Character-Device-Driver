...
static int	globalvar_init(void)   //字符设备模块初始化
{
	int result = 0; 
    int err = 0;
	dev_t dev = MKDEV(major, 0); // 将主、次设备号拼凑为设备号
	//虽然得到了设备号，但还没有注册到内核，下一步申请
    if (major)
	{
        //静态申请设备编号
	...
	if (result < 0)  //申请失败
		return result;
	cdev_init(&globalvar.devm, &globalvar_fops);   //初始化字符设备
    ...
    device_create(my_class, NULL, dev, NULL, "chardev0"); //设备创建成功
    return 0;
}
